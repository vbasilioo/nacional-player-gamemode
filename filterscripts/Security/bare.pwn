...
public OnPlayerConnect(playerid) {
	...
	if (!SvGetVersion(playerid)) SendClientMessage(playerid, -1, "{FF0000}[NP VOIP] {FFFFFF}Voip não foi carregado.");
	else if (!SvHasMicro(playerid)) SendClientMessage(playerid, -1, "{32CD32}[NP VOIP] {FFFFFF}Microfone não foi reconhecido.");
