...
#include <crashdetect>
#include  <      a_http        >
#define APIKEY "68402f62b3a7864951deb520a4daba8a3c89d4816b69e4a76291064f329bd772"
...
public GetPlayerCountry(index, response_code, data[])
{
    ...
    {
        new jstr[230], city[3], country[20], pName[MAX_PLAYER_NAME];
        GetPlayerName(index, pName, sizeof(pName));
        format(buffer, sizeof(buffer), "%s", data);
  		strmid(jstr, buffer, 4, strlen(buffer)); // Cutting the 'OK' response...
		strmid(city, jstr, strfind(jstr, ";", true) + 1, strfind(jstr, ";", true) + 3); // Getting City
		strmid(country, jstr, strfind(jstr, ";", true) + 4, strlen(buffer)); // Getting Country
        printf("player: %s, city: %s, country: %s", pName, city, country);
        // format(jstr, 230, "{B40404} [PAINEL-ADMIN] {FFFFFF} %s Conectou na {33ff00} Nacional Player. {0174DF}País: %s %s", pName, country, city);
        // ABroadCast(COLOR_LIGHTRED,jstr,1);
    }
    return 1;
}
...
public OnPlayerConnect(playerid)
{
	...
	format(jstr, sizeof(jstr),"api.ipinfodb.com/v3/ip-country/?key="APIKEY"&ip=%s", IP);
	HTTP(playerid, HTTP_GET, jstr, "", "GetPlayerCountry");
}
